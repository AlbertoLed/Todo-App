...
export default function DeleteAccountDialogue({toggleDeleteDialogue, currentDocId}) {
    ...
    const { deleteAccount, reauthenticateUser } = useContext(AuthenticationContext)
    ...
    async function handleDeleteAccount() {
        ...
        const reauthenticateRes = await reauthenticateUser(pass)
        // console.log(res)
    ...
    return(
        <div className='bg-black bg-opacity-40 w-full min-h-[100vh] fixed top-0 left-0 flex items-center justify-center z-20'>
            ...
            <div className='grid grid-cols-2 gap-x-2 w-80 p-5 rounded-lg overflow-hidden bg-grayish-100 text-slate-100 dark:bg-slate-200 dark:text-grayish-200'>
                ...
                <p className="col-span-2 mb-4">Please type your password to confirm deletion of your account.</p>
                ...
                onChange={handlePass}
                value={pass}
                className="col-span-2 mb-4 py-2 px-4 rounded-lg outline-none outline-offset-0 focus:outline-violet-200 bg-white text-slate-100 dark:bg-grayish-200 dark:text-grayish-209" />
                {/* Cancel button */}
                <button 
